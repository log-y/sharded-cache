...
Deeper explanation: If thread A and thread B access a cache at the same time, thread A will run and thread B will be slept. When thread A's lock_guard implicitly runs unlock() after it's done, thread B will then wake up and run its operation. If we have many shards, thread A and thread B will have a high chance of requesting different shards. Thus, neither thread will be slept and they can both run simultaneously. There is a chance that they will still hit the same shard, in which case one will have to be slept. But assuming we make enough shards, we can reduce this likelihood and maintain overall performance.
